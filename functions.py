...
def interpolate_nans(x):
    ...
    nans = np.isnan(x) #tableau de True/False
...
def median_4_bis(all_psd, all_dates, freqs,limx=None, we=False, eq = False, p=80 ,n=5, outfile=None):
    ...
    # Création de 3 sous-graphiques (3 lignes, 1 colonne)
...
def median_4(all_psd, all_dates, freqs,limx=None, limy=(-200,-50), we=False, eq = False):
    plt.figure(figsize=(10, 6))

    if we == True:
        add_weekends(datetime(y,1,1),datetime(y+1,1,1))
    if eq == True:
        
        add_eq_med(datetime(y,1,1),datetime(y+1,1,1))
    band_list=[(0.01,0.1),(0.1,1),(1,5),(5,50)]
    
    y=all_dates[0].year
    y_list = get_medians_4(all_psd, all_dates, freqs)
    for y_psd,band in zip(y_list,band_list):
        plt.plot(all_dates, y_psd, label=f"{band[0]}-{band[1]} Hz", alpha = 0.5)

    plt.ylabel("Médiane des PSD (dB)", size=10)
    ...
    if limx:
        ...
        plt.xlim(datetime(y,1,1),datetime(y+1,1,1))
    c,d = limy
    plt.ylim(c,d)
    #plt.title(f"Evolution of median PSD with time", size =20)
    add_legend(we,eq)
...
def get_anomaly(dates,data, p, n):
    '''
    dates = dates associées aux datas
    data = données à analyser
    p = percentile au-delà duquel on considère une anomalie
    n = nombre d'heure consécutives minimum pour définir une période d'anomalie
    '''
...
def get_data(catalog,instrument=None):
    ...
    for i in tqdm(range(len(df))):
        detect = get_detector_with_retry(
            df.iloc[i]["jsonurl"],3,i
        )
    ...
    if instrument :
        ...
        df_inst[f"day_or_night_V1"] = df_inst["date"].apply(day_or_night)
        print("Fini !")
...
def plot_events(df,run,offline_periods=None, lim = None,outfile=None):
    ...
    if df.empty:
        ...
        min =df["network_matched_filter_snr"].min()
        max =df["network_matched_filter_snr"].max()
    ...
    Line2D([0], [0], marker='o', color='black',
        ...
        / (max - min)
           ...
           linestyle='',
           markersize=10, label=f"Min = {min}"),

    Line2D([0], [0], marker='o', color='black',
            alpha=0.2 + 0.8 * (
        (np.round((max+min)/2) - min)
        / (max - min)
        ),
           linestyle='',
           markersize=10, label=f"Moyenne = {(max+min)/2}"),
    ...
    loc="center left",
    bbox_to_anchor=(1, 0.5), labelspacing = 1, fontsize=10)
...
def plot_days(df):
    ...
    min = df["date"].iloc[-1]
    max = df["date"].iloc[0]
    plt.title(f"Nombre de détections par Virgo par jour de la semaine pour la période {min} - {max}")
    plt.show()
...
def plot_sensitivity(df,type="BNS",xlim=None,ylim=None,we=False, eq = False,outfile=None):
    ...
    for i, row in df_new.iterrows():
        ...
        if i != df_new.index[-1]:
            next_row = df_new.iloc[df_new.index.get_loc(i)+1]
            x.append(next_row["UTCstart"])
            y.append(next_row[type])
...
def plot_bns_param(df,param_1,param_2,xlim=None,ylim=None,dt=0.1,window=1):
    range,median_profile = get_median_profile(df,param_1,param_2,dt,window)
    plt.scatter(df[param_2],df[param_1],s=8, alpha=0.7)
...
#STRAIN TIMESERIES
def cut_in_h_timeseries(strain, start, end):
...
def pad_to_full_hour_timeseries(strain_hour):
...
def get_periodogram_strain(
    ...
    x, fs = pad_to_full_hour_timeseries(strain_hour)
...
def PSD_strain_by_h(strain, start, end):
    ...
    strain_hours = cut_in_h_timeseries(strain, start, end)
    ...
    return (
        np.array(psd_list),
        np.array(freq_list),
        pd.to_datetime(date_list, utc=True)
    )
...
def spec_plot_strain(
        all_psd,
        dates,
        freqs,
        fmin=0,
        fmax=1000,title=None,log=True,outfile=None):
...
def get_medians_strain(all_psd, freqs):
    ...
    for band in band_list:
        ...
        for psd in all_psd:
            ...
            median_psd.append(
                np.median(psd[freq_band])
            )

        y_list.append(median_psd)

    return band_list,y_list
def median_4_strain(all_psd, all_dates, freqs,limx=None, we=False, eq = False, p=80 ,n=5,outfile=None):
    y=2024
